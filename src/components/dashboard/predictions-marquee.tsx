...
function PredictionItem({ p }: { p: PredictionRow }) {
  return (
    <span className="flex items-center gap-2.5 px-7">
      ...
      <span className="font-mono text-[0.9rem] font-semibold tabular-nums text-feed-warn">{p.yesPercent}%</span>
      <span aria-hidden className="ml-3 size-1 rounded-full bg-overlay-strong" />
    </span>
  );
}

const LABEL = "z-20 flex h-full flex-none items-center bg-background px-4 text-[0.64rem] font-semibold uppercase tracking-[0.16em] text-muted-foreground";
...
export function PredictionsMarquee() {
  ...
  return (
    <div className="mb-marquee relative flex h-12 flex-none items-center overflow-hidden bg-overlay-weak">
      {/* fixed bookends mirroring each other */}
      <span className={`${LABEL} absolute left-0 top-0 border-r border-hairline`}>Predictions</span>
      ...
      <span className={`${LABEL} absolute right-0 top-0 border-l border-hairline normal-case tracking-[0.04em]`}>via Polymarket</span>
    </div>
  );
}
